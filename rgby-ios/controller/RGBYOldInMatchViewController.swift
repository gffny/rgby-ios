...
class RGBYOldInMatchViewController: UIViewController, UITableViewDataSource, RGBYMatchDetailDelegate {
    ...
    @objc func handleClockAction(_ sender:RGBYMatchClock) {
        ...
        if sender.clockActionButton.titleLabel?.text == RGBYOldInMatchViewController.CLOCK_START_MATCH {
            self.matchDetail.startPeriod()
        } else if sender.clockActionButton.titleLabel?.text == RGBYOldInMatchViewController.CLOCK_SECOND_HALF {
            self.matchDetail.startPeriod()
        } else {
            self.matchDetail.stopPeriod()
            if self.matchDetail.currentPeriod == 1 {
                sender.setClockText(text: RGBYOldInMatchViewController.CLOCK_SECOND_HALF)
            }
        }
    }
...
//        performSegue(withIdentifier: "presentModeSwitch", sender: self)
    ...
    func setupMatchClock() {
        ...
        self.matchClock.setClockText(text: RGBYOldInMatchViewController.CLOCK_START_MATCH)
    }
